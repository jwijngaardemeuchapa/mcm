...
## 2026-09-08 — Release v1.0.63

**Actor:** Jeremiah | **Agent:** sonnet
**Summary:** Última release da main foi 1.0.62 (19/08) — vários commits reais ficaram sem empacotar: lista de bots FUP/BID ao vivo da Umbler, "Nome + CPF de todos" resgatado no menu Copiar, fixes da Troca de Turno (empresas sumindo da Carteira sem aviso, confirmações comparando contra pedido original em vez de quem foi escalado) + PréFUP unificado em 5h, e a remoção do `central.ts` morto (não muda comportamento, era código nunca importado). Version bump 1.0.62→1.0.63 (`tauri.conf.json`, changelog em `Ajuda.tsx`), `createUpdaterArtifacts: true` adicionado ao bundle config (faltava — ver LESSONS.md 2026-09-08 e o mesmo achado no release da beta hoje), `npx tauri build` com a chave de assinatura já saiu com `.sig` de primeira desta vez. `latest.json` atualizado.
**Files changed:** `src-tauri/tauri.conf.json`, `src/pages/Ajuda.tsx`, `latest.json`
**Next:** publicar o release `v1.0.63` no GitHub com os artefatos.
...
## 2026-09-17 — Push pra Central agora também na produção (não só beta)

**Actor:** Jeremiah | **Agent:** sonnet
**Summary:** Usuário perguntou se a Central já recebe sim/não de FUP/PréFUP independente de quem tá na beta. Resposta encontrada: `syncFirestoreStatus` da Central lê o Firestore direto (não depende do MCM), mas é poll de 5min — perde a corrida quase sempre contra o `deleteDoc` quase-instantâneo que o MCM faz na mesma mensagem assim que processa (`useFirestoreQueue.ts`). Usuário apontou exatamente isso ("a central precisaria ler em tempo real, pois se o MCM apagar o registro, a central nem sabe") e pediu pra implementar dispatch_events + confirmado_via também na main. Em vez de fazer a Central tentar ler mais rápido (impossível, é poll serverless), portei da beta pro main o PUSH em tempo real — o MCM avisa a Central no mesmo instante que processa, antes de apagar a mensagem. Só a direção push foi portada (nunca pull/leitura da Central), respeitando a regra "produção nunca depende da Central" (ver `feedback_beta_vs_producao` na memória): `src/lib/central.ts` novo (só pushDispatchEventToCentral/pushChapaStatusToCentral/pushRespostaToCentral, sem nenhum pull), `canalConfirmacao()` portado pro `prefup.ts` (main já tinha `fup_log.aguarda_resposta_chat`, só faltava a função), migration v24 (`chapas.confirmado_via`), `firestoreQueue.ts` com os mesmos pushes que a beta já tinha em `processFirestoreMessage`, `dispatchQueue.ts` com `pushDispatchEventToCentral` em todo ponto de disparo (mesmos canais da beta: umbler_prefup/umbler_talk/bid/cancelamentos). NÃO portado: `upsertChatLink`/chat_links (feature separada, pull-based, seria quebra de regra) e os botões manuais de confirmar em TaskCard/TaskDetailPanel (fora do escopo pedido — só o fluxo automático via bot). Endpoints da Central (`chapa-dispatch`, `chapa-status`, `resposta`) já existiam prontos, nenhuma mudança no central-hub foi necessária. Typecheck (baseline 13, nenhum erro novo) e `cargo check` limpos.
**Files changed:** `src/lib/central.ts` (novo), `src/lib/prefup.ts`, `src/lib/firestoreQueue.ts`, `src/lib/dispatchQueue.ts`, `src-tauri/src/lib.rs`
**Next:** ainda não empacotado em release — perguntar ao usuário se quer isso publicado já ou junto de outras mudanças pendentes. Se quiser espelhar confirmações MANUAIS (clique em TaskCard/TaskDetailPanel) também na main, é trabalho futuro não escopado aqui.

## 2026-09-12 — Release v1.0.65: fix chat (CSP), bug real do CPF list, envio pro grupo, copiar config entre instalações

**Actor:** Jeremiah | **Agent:** sonnet
**Summary:** Sessão longa, vários itens do usuário resolvidos em sequência. (1) "imagem não abre, áudio não toca no chat" — causa raiz achada por inspeção estática: `img-src`/`media-src` do CSP (`tauri.conf.json`) só permitiam `self`/`data:`/`blob:`, sem `https:` — qualquer `<img>`/`<audio>` apontando pra URL externa da Umbler era bloqueado pelo próprio webview antes da requisição. `connect-src` já tinha `https:` genérico; estendido o mesmo padrão. Aplicado em main e beta. (2) Usuário pediu de volta a lista "Nome + CPF" formatada — investigação achou que ela JÁ existia (`copyAllNamesAndCpf`/`copyCpfConfirmados` no menu "Copiar" de TaskCard/TaskDetailPanel), só que o rótulo "CPFs dos confirmados" não deixava claro que incluía nome, então parecia ausente. Rótulo corrigido pra "Nome + CPF dos confirmados". No processo, achado um bug REAL: a implementação de `copyCpfConfirmados` no `TaskDetailPanel.tsx` (Panorama/Timeline) tinha o MESMO NOME da função do `TaskCard.tsx` (Cards) mas devolvia só dígitos crus do CPF, sem nome e sem `formatCpf()` — usuário reportou exatamente isso ("não vem formatado como antes"). Reescrita pra bater com o TaskCard. Aplicado em main e beta. (3) Usuário pediu enviar a lista direto pro grupo do WhatsApp do cliente em vez de só copiar — nova opção "Enviar lista de confirmados pro grupo" no TaskDetailPanel, usa `ConversationPane.fillReply()` (existia desde sempre, nunca tinha consumidor) pra preencher o composer sem enviar sozinho — analista revisa e aperta Enter, mesma regra de todo atalho de texto do componente. (4) Usuário pediu a beta herdar tudo configurado no main instalado — `exportSettingsJson()`/`importSettingsJson()` em `settings.ts` + card em Integrações ("copiar configurações entre instalações"): cada instalação Tauri tem localStorage isolado (identifier diferente = perfil WebView2 separado), então não tinha outro jeito de levar isso de uma pra outra sem passar pelo próprio app. Tudo aplicado em main (release v1.0.65) e beta (release beta-v1.0.68).
**Files changed:** `src-tauri/tauri.conf.json` (main+beta), `src/components/TaskCard.tsx` (main+beta), `src/components/TaskDetailPanel.tsx` (main+beta), `src/lib/settings.ts` (main+beta), `src/pages/Integracoes.tsx` (main+beta), `src/pages/Ajuda.tsx`, `latest.json`, `latest-beta.json`
**Next:** nenhum pendente destes itens.

## 2026-09-12 — Gatilho de limpeza de etiquetas do Umbler Talk (Central) + chat_links portado pro main

**Actor:** Jeremiah | **Agent:** sonnet
**Summary:** Usuário pediu um gatilho pra remover TODAS as etiquetas do Umbler Talk assim que uma tarefa entra "Em Andamento". Confirmado no Swagger oficial da Umbler (não documentado em nenhuma referência prévia) que existe `DELETE /v1/chats/{id}/tags/list/` (body: `{organizationId, tagIds}`). Implementado do lado da **Central** (`central-hub`, `syncTarefas()` em `sync.server.ts`): a cada sync de 5min, resolve `chat_links` (tarefa/chapa → `umbler_chat_id`) das tarefas recém-"Em Andamento", busca as tags atuais via `GET /v1/chats/{id}/` e remove tudo. Nova coluna `tarefas.tags_removidas_em` torna idempotente (nunca repete). Migration aplicada em produção via prompt ao Lovable, verificada ao vivo (coluna existe, `curl` confirmado). **Achado crítico na verificação:** `chat_links` estava **completamente vazia** em produção — a inserção de teste que fiz virou literalmente `id:1`, a primeira linha da tabela desde sempre. Causa: `upsertChatLink`/push pra Central só existia na **beta** (`chatLinks.ts`), e o disparo real do dia a dia acontece majoritariamente pela **main** — a feature de etiquetas nunca teria efeito prático sem isso. Usuário confirmou portar pro main: novo `src/lib/chatLinks.ts` (idêntico ao da beta, só a direção push — sem `pullChatLinksFromCentral`/`applyChatLinksLocally`, que continuam exclusivos da beta), `pushChatLinkToCentral` em `central.ts`, migration v25 (`chat_links`, mesma tabela da beta) em `lib.rs`, e `upsertChatLink()` chamado nos mesmos 6 pontos de disparo que a beta já tinha (`dispatchQueue.ts`). Linha de teste (`id_tarefa=999999999`) ficou órfã em produção — a chave anon não tem DELETE, avisado ao usuário com o SQL pra limpar via Lovable se quiser.
**Files changed:** central-hub: `src/lib/sync.server.ts`, `supabase/migrations/20260912140000_tarefas_tags_removidas_em.sql` (novo). mcm main: `src/lib/chatLinks.ts` (novo), `src/lib/central.ts`, `src/lib/dispatchQueue.ts`, `src-tauri/src/lib.rs`
**Next:** confirmar com uma tarefa real entrando "Em Andamento" (com chat_link já populado por um disparo real) que as etiquetas somem de fato — ainda não verificado ponta-a-ponta com dado real, só a infra. Limpar a linha de teste `id_tarefa=999999999` em `chat_links` se o usuário quiser (SQL já entregue).

## 2026-09-12 — Release v1.0.66: copiar configurações + chat_links na produção

**Actor:** Jeremiah | **Agent:** sonnet
**Summary:** Empacota os 2 últimos commits do main sem release ainda: exportSettingsJson/importSettingsJson (Integrações) e chat_links portado (ver entradas acima). Version bump 1.0.65→1.0.66, changelog em `Ajuda.tsx`, build assinado (`.sig` automático de primeira). `latest.json` atualizado e verificado (200/200 no updater endpoint e no download do GitHub Release).
**Files changed:** `src-tauri/tauri.conf.json`, `src/pages/Ajuda.tsx`, `latest.json`
**Next:** nenhuma pendência de release aberta na main. Confirmar com o usuário se a remoção de etiquetas (item acima) funcionou na prática assim que houver uma tarefa real entrando "Em Andamento" com chat vinculado.

## 2026-09-15 — Incidente + fix urgente: React error #310 ao abrir card de tarefa (v1.0.67)

**Actor:** Jeremiah | **Agent:** sonnet
**Summary:** Usuário reportou que abrir qualquer card de tarefa quebrava com "Minified React error #310" — Rules of Hooks violado. Causa raiz: o `useEffect` do fillReply (feature "Enviar lista de confirmados pro grupo", introduzida nesta sessão na v1.0.65) tinha sido colocado DEPOIS do `if (!task) return null;` no `TaskDetailPanel.tsx` — quando `task` fica `null` numa renderização (fechar o painel, trocar de tarefa), o hook deixa de rodar e o React perde a contagem de hooks entre renders ("Rendered more hooks than during the previous render"). Bug meu, não pego no typecheck (TypeScript não valida Rules of Hooks — precisaria de `eslint-plugin-react-hooks` rodando, que não bloqueia o build aqui). Fix: `useEffect` movido pra antes do early return, junto dos outros hooks do componente — mesmo padrão de todos os outros hooks já existentes ali. Release de emergência: v1.0.67 (main) e beta-v1.0.69 (beta, mesmo bug portado junto na sessão anterior), as duas verificadas (200/200 no updater e no download).
**Files changed:** `src/components/TaskDetailPanel.tsx`, `src-tauri/tauri.conf.json`, `src/pages/Ajuda.tsx`, `latest.json`
**Next:** nenhuma pendência de release aberta. Ideia registrada (não implementada): adicionar `eslint-plugin-react-hooks` ao lint/typecheck do CI pra pegar esse tipo de bug antes de chegar em produção.
