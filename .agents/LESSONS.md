...
## 2026-07-16 [migrations, cross-repo, shared-db, sqlx]
**Rule:** `mcm` (v1) e `mcm-v2` compartilham o MESMO banco físico (`fupmanager.db`, resolvido via pasta irmã), mas cada repo tem sua PRÓPRIA lista de migrations Rust, numeradas independentemente. NUNCA escolher um número de `version` para uma migration nova sem antes checar `grep "version: " src-tauri/src/lib.rs` no OUTRO repo.
**Why:** Colisão real encontrada (não hipotética): v1 tem `version: 15` = `activity_log` com colunas `descricao/chapa_nome/empresa/timestamp`; mcm-v2 tem `version: 16` = `activity_log` com colunas DIFERENTES `mensagem/created_at`. Mesmo nome de tabela, schemas incompatíveis. Com `CREATE TABLE IF NOT EXISTS`, quem rodar primeiro numa máquina "vence" — o outro app marca a migration como aplicada em `_sqlx_migrations` mas a tabela real não tem as colunas que seu código espera. Provável erro silencioso na feature de activity feed da MV2 se ela já rodou numa máquina com o v1 instalado.
**How to apply:** Antes de adicionar uma migration em qualquer um dos dois repos, rodar `grep "version: " src-tauri/src/lib.rs` NO OUTRO repo e escolher um número que não colida com nenhum dos dois (ex.: MCM-97 usou `version: 19` porque v1 estava em 15 e mcm-v2 já tinha ido até 18). Ainda não corrigido: reconciliar o `activity_log` que já colidiu, e definir uma regra permanente (faixas reservadas por repo, ou um único ponto de coordenação) — decisão pendente do usuário.

---

## 2026-08-19 [sqlite, tauri-plugin-sql, transactions, locking]
