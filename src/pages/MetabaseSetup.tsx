...
import { Database, RefreshCw, Eye, Search, CheckCircle2, KeyRound } from "lucide-react";
 ...
 * Tela temporária do M1: vira a config oficial de fonte de dados na UI nova.
 */
...
export default function MetabaseSetup() {
  ...
  async function salvar() {
    ...
    try {
      ...
      setApiKey(""); // não mantém a chave em memória do frontend
      ...
      {cards.length > 0 && (
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          ...
          <div className="max-h-[480px] overflow-auto divide-y divide-border">
            {filtered.map((c) => (
              <div key={c.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/20">
